...
#include "Level.h"
/*
	Create the asteroid, using the screen dimensions for positionign and scale
*/
Asteroid* Asteroid::create(cocos2d::Size res) {
	...
	if (asteroid && asteroid->initWithFile("Asteroid.png")) {					// Create asteroid from .png in resources
		...
		asteroid->setPosition(res.width + asteroid->getContentSize().width, res.height * asteroid->m_randYPos);// Position to spawn the power up at
	}
	else {
		delete asteroid;
	}
