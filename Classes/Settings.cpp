...
bool Settings::init() {
	...
	instructionLbl = cocos2d::Label::createWithTTF("Adjust Audio Volumes", 
	...
	instructionLbl = cocos2d::Label::createWithTTF("Select Music Track", 
		"fonts/Super Mario Bros..ttf", visibleSize.height * 0.045f);															// Select track label
	...
	// Track Backwards
	btnTrackDownImg = MenuItemImage::create("btnMinus.png", "btnMinusSelect.png",
		CC_CALLBACK_1(Settings::skipTrackBackwards, this));																		// Set image for menu option
	...
	// Track Forwards
	btnTrackUpImg = MenuItemImage::create("btnPlus.png", "btnPlusSelect.png",
		CC_CALLBACK_1(Settings::skipTrackForwards, this));																		// Set image for menu option
	...
	// Current Track
	currentTrackLbl = cocos2d::Label::createWithTTF(Audio::Instance()->getTrackName(), 
		"fonts/Super Mario Bros..ttf", visibleSize.height * 0.045f);															// Show the current tracks name
...
void Settings::musicVolUp(cocos2d::Ref *sender) {
	Audio::Instance()->musicVolIncrease();																						// Increase the music volume
}
void Settings::musicVolDown(cocos2d::Ref *sender) {
	Audio::Instance()->musicVolDecrease();																						// Decrease the music volume
}
void Settings::sfxVolUp(cocos2d::Ref *sender) {	
	Audio::Instance()->sfxVolIncrease();																						// Increase SFX volume
}
void Settings::sfxVolDown(cocos2d::Ref *sender) {
	Audio::Instance()->sfxVolDecrease();																						// Decrease SFX volume
}
void Settings::skipTrackForwards(cocos2d::Ref *sender) {
	...
	Audio::Instance()->skipTrackForwards();																						// Skip the track forwards
	currentTrackLbl->setString(Audio::Instance()->getTrackName());																// Display the track name
}
void Settings::skipTrackBackwards(cocos2d::Ref *sender) {	
	Audio::Instance()->skipTrackBackwards();																					// Select previous track
	currentTrackLbl->setString(Audio::Instance()->getTrackName());																// Display track name
}
...
void Settings::initBackground() {
	...
	backgroundSprite = Sprite::create("BG1080p.png");																			// Decide which background image to use, based on the platforms screen resolution
	backgroundSprite->setScale(visibleSize.height == 1080 ? 1.0f : 0.67f);														// Scale the image
