...
HUD *HUD::create(cocos2d::Point position, cocos2d::Size res) {
	...
	if (true) {
		...
		bottomPanel->setPosition(cocos2d::Point(res.width / 2, 
			(bottomPanel->getContentSize().height / 2) * scaleDown));
		...
		s_pInstance->m_pMenuItem->setPosition(cocos2d::Point(
			(position.x + res.width * 0.05f) * 0.99f, res.height * 0.95 + position.y));

		// Current Level
		s_pInstance->m_pLevelLabel = Label::createWithTTF(tempLevel->getCString(), 
			"fonts/Marker Felt.ttf", res.height * 0.05f);
		...
		// Current score
		s_pInstance->m_pScoreLabel = Label::createWithTTF(tempScore->getCString(), 
			"fonts/Marker Felt.ttf", res.height * 0.05f);
		...
		// Countdown Timer
		s_pInstance->m_pTimeLabel = Label::createWithTTF(tempTime->getCString(), 
			"fonts/Marker Felt.ttf", res.height * 0.05f);
		...
		s_pInstance->m_pCloseItem->setPosition(cocos2d::Point((position.x + res.width - 
			s_pInstance->m_pCloseItem->getContentSize().width / 2) * 0.975f, res.height * 0.05f));
...
void HUD::update(float curTimeMillis) {
	...
	/*
		Added logTiming() to register time player exits game with close button
	*/
	...
	/*
		Added logTiming() to register time player exits game to the menu
	*/
